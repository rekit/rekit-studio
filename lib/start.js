...
const plugins = ['../src/features/plugin-cra', '../src/features/plugin-node'];
plugins.forEach(p => {
  const pkgJson = require(path.resolve(p, 'package.json'));
  if (_.castArray(rekit.core.config.getRekitConfig().appType).indcludes(pkgJson.appType)) {
    const pluginInstance = require(path.resolve(p, 'core'));
    pluginInstance.root = path.resolve(__dirname, p);
