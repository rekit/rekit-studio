...
process.env.REACT_APP_REKIT_TOKEN = token;
...
function startDevServer(args) {
  ...
  checkBrowsers(paths.appPath, isInteractive)
    .then(() => {
      ...
      // Authentication
      devServer.app.use(auth.validator(token));
      // should not put in devserverConfig.before, why?
      // configStudio(devServer.listeningApp, devServer.app, devServer);

      // Launch WebpackDevServer.
      const ss = devServer.listen(port, HOST, err => {
        if (err) {
      ...
      configStudio(devServer.listeningApp, devServer.app, ss);
